...
namespace Enemy
{
    public class SniperAI : MonoBehaviour
    {
        ...
        [SerializeField] private float checkFrequence;
        ...
        void OnEnable()
        {
            _currentState = StartCoroutine(nameof(Vigilant));
            
        }

        private IEnumerator Vigilant()
        {
            while (true)
            {
                ...
                for (int i = 0; i < nbInRange; i++)
                {
                    ...
                    {
                        go = rb.gameObject;
                        if (rb.gameObject.CompareTag("Friendly"))
                        {
                            ...
                            foreach (var point in extremities)
                            {
                                ...
                                bool intersect = Physics.Raycast(origin, direction, direction.magnitude, _obstacleLayersMask, QueryTriggerInteraction.UseGlobal);
                                ...
                                if (!intersect)
                                {
                                    ...
                                    _currentState = StartCoroutine(AimAt(go));
                                    yield break;
                                }
                            }
                        }
                ...
                yield return new WaitForSeconds(checkFrequence);   
            }
        }
